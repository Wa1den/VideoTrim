    ...
    public static async Task<Result> Check()
    {
        try
        {
            ...
            if ((int)response.StatusCode == 404)
                return new Result(null, Page, "релизов на GitHub пока нет, или репозиторий закрыт");
            if (!response.IsSuccessStatusCode)
                return new Result(null, Page, $"GitHub ответил {(int)response.StatusCode} {response.ReasonPhrase}");
            ...
            if (!TryRead(tag, out var latest)) return new Result(null, url, "не разобран тег релиза " + tag);
        ...
        {
            return new Result(null, Page, ex is TaskCanceledException ? "GitHub не ответил за 10 секунд" : ex.Message);
        }
    }
