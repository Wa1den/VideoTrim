...
public sealed class AboutWindow : Window
{
    ...
    public AboutWindow(Window owner, string? ffmpeg, Config cfg, UpdateCheck.Result? known)
    {
        ...
        name.Children.Add(new TextBlock { Text = "Версия " + Version, FontSize = 12, Foreground = Ui.FgDim });
        ...
        panel.Children.Add(Para(
            "Вырезает фрагмент из видео через ffmpeg. Разрешение, частота кадров, кодек и битрейт "
            + "по умолчанию остаются исходными, кодек и битрейт можно сменить. Фрагмент режется "
            + "точно по секундам с перекодированием или без потерь по ключевым кадрам."));
        panel.Children.Add(Para(ffmpeg != null ? "ffmpeg: " + ffmpeg : "ffmpeg не найден"));

        var repo = Para("Исходный код: ");
        ...
        var auto = new CheckBox { Content = "Проверять обновления при запуске", MinWidth = 0, IsChecked = cfg.CheckUpdates };
        ...
        var now = Ui.Btn("Проверить сейчас", async () => await CheckNow());
        ...
        var close = Ui.Btn("Закрыть", Close);
        close.IsCancel = true;
    ...
    void ShowResult(UpdateCheck.Result r)
    {
        ...
        if (r.Newer != null)
        {
            _update.Inlines.Add($"Доступна версия {r.Newer}: ");
            _update.Inlines.Add(Link("страница релиза", r.Url));
        }
        else if (r.Error != null) _update.Text = "Проверить не удалось: " + r.Error;
        else _update.Text = "Установлена последняя версия";
    }
